app.controller('fornecedorController', function ($http, $scope) {
   $scope.runUpload = 0;
   // quando seleciona a imagem executa automatico
    ...
    $scope.dados = {};
    $scope.urlpage = "/admin/fornecedor/index";
    ...
    $scope.store = function () {
        if($scope.runUpload == 0){
            ...
            $http({
                ...
                if(response.data.errors.cell_phone){
                    mensagem += response.data.errors.cell_phone+ '<br>';
                }
                if(response.data.errors.id_activity){
                    mensagem += response.data.errors.id_activity+ '<br>';
                }
                if(response.data.errors.cep){
                    mensagem += response.data.errors.cep+ '<br>';
                }
                if(response.data.errors.street){
                    mensagem += response.data.errors.street+ '<br>';
                }
                if(response.data.errors.number){
                    mensagem += response.data.errors.number+ '<br>';
                }
                if(response.data.errors.id_city){
                    mensagem += response.data.errors.id_city+ '<br>';
                }
    ...
    $scope.edit = function (id) {
        ...
        $http({
            url: '/admin/fornecedor/editAngular/' + id,
            data: $scope.dados,
            method: "GET"
        }).then(function (response) {
    ...
    $scope.update = function (id) {
        if($scope.runUpload == 0){
            ...
            $http({
                url: '/admin/fornecedor/update/'+id,
                data: $scope.dados,
                method: "POST"
            }).then(function (response) {
                console.log(response);
